...
for pos, song in enumerate(filenames):
    ...
    if filetype == 'flac':
        render_video = f"ffmpeg -hide_banner -loglevel warning -loop 1 -i \"{cover_path}\" -i \"{song_path}\" -c:a aac -ab 320k -c:v libx264 -shortest -strict -2 \"{video_output_path}\""
    else:
        render_video = f"ffmpeg -hide_banner -loglevel warning -loop 1 -i \"{cover_path}\" -i \"{song_path}\" -c:a copy -c:v libx264 -shortest \"{video_output_path}\""
